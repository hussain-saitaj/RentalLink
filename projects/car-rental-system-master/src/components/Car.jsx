...
const Car = () => {
    ...
    const fetchCarsData = async () => {
        ...
        try {
            if (user) {
                const q = user.role === 2 ? query(collection(firestore, "cars")) : query(collection(firestore, "cars"), where("oid", "==", user.uid));
    ...
    const filterCars = (brand,type,price) =>{
        ...
        const filtered = cars.filter(car => 
            ...
            car.type.toLowerCase().includes(type.toLowerCase()) && (price===""?"123456789".includes(car.pph):price.includes(car.pph))
        );
    ...
    useEffect(() => {
        fetchCarsData();
    }, [user]); // Dependencies should ensure this only runs when necessary
